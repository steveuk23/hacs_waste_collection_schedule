...
TEST_CASES = {
    ...
    "Hadfield Conservative Club, 221 Woolley Bridge Road, Hadfield, SK13 1BW": {
        "PAGESESSIONID": "71b6cce0-06e1-43e5-999c-56e4f84e3142&fsn=2306292b-e07d-4ba9-b899-ea50d0c98a70"
    },
...
class Source:
    def __init__(
        ...
        self._PAGESESSIONID = PAGESESSIONID
        self._post_code = post_code
        self._house_number = house_number
        if not any([self._PAGESESSIONID, self._post_code and self._house_number]):
            raise Exception(
                "PAGESESSIONID or post_code and house number must be provided in config"
            )
    ...
    def fetch(self):
        ...
        if self._PAGESESSIONID is not None:
            r = requests.get(
                "https://www.highpeak.gov.uk/findyourbinday",
                params={
                    "PAGESESSIONID": self._PAGESESSIONID,
                },
            )
        else:
            # Property search endpoint redirects you to the page, so by caching
            # The PAGESESSIONID in future, we save an extra request every check.
            r = requests.get(
                "https://www.highpeak.gov.uk/findyourbinday",
                params={
            ...
            self._PAGESESSIONID = params["PAGESESSIONID"].pop()
